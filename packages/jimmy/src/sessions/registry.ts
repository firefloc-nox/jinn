...
// ── Session events (activity panel replay) ───────────────────────────
...
/**
 * Enforce per-session event limit by deleting oldest events beyond the cap.
...
/**
 * Enforce global event limit by deleting oldest events across all sessions.
 */
export function trimGlobalEvents(maxGlobal: number): void {
  if (maxGlobal <= 0) return;
  const db = initDb();
  db.prepare(`
    DELETE FROM session_events WHERE id NOT IN (
      SELECT id FROM session_events ORDER BY timestamp DESC LIMIT ?
    )
  `).run(maxGlobal);
}
